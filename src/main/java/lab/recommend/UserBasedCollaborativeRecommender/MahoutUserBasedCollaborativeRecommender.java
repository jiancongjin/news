 ...
 * 
 */
package lab.recommend.UserBasedCollaborativeRecommender;
	...
	public void recommend(List<Long> users,int isFirst)
	{
		...
		try
		{
			...
			List<NewsLogs> newslogList=NewsLogs.dao.find("select " + DBKit.PREF_TABLE_USERID + ","
					+ DBKit.PREF_TABLE_NEWSID + "," + DBKit.PREF_TABLE_TIME + " from news_logs");
		...
		System.out.println("CF has contributed " + Math.ceil(count*1.0/users.size()) + " recommending news on average");
