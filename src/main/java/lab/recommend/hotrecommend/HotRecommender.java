 ...
 * 
 */
package lab.recommend.hotrecommend;
 ...
 * @date 2019年4月30日 基于“热点新闻”生成的推荐，一般用于在CF和CB算法推荐结果数较少时进行数目的补充
 */
public class HotRecommender implements RecommendAlgorithm
{
	...
	public static int TOTAL_REC_NUM = 200;
	...
	public void recommend(List<Long> users,int isFirst)
	{
		...
		int count=0;
		Timestamp timestamp = getCertainTimestamp(0, 1, 0);
		for (Long userId : users)
		{
			try
			{
				...
				if(recommendation!=null) {
					tmpRecNums = recommendation.getLong("recnums");
				}
				int delta= flag? (int) (TOTAL_REC_NUM - Long.valueOf(tmpRecNums.toString())) :TOTAL_REC_NUM;
		...
		//double recommendCount  = Math.ceil(count*1.0/users.size());
		System.out.println("HR has contributed " + (users.size()==0?0:Math.ceil(count*1.0/users.size())) + " recommending news on average");
	...
	public static void formTodayTopHotNewsList()
	{
		...
		try
		{
			List<NewsLogs> newslogsList= NewsLogs.dao.find("select news_id,count(*) as visitNums from news_logs where view_time>"
							+ RecommendKit.getInRecDate(beforeDays) + " group by news_id order by visitNums desc");
			//System.out.println(Arrays.toString(new List[]{newslogsList}));
